...
std::vector<uint8_t> cutKey(const std::string& username, const std::string& ip)
{
	...
	data.resize(0x50);
	memcpy(&data[0], username.c_str(), std::min(31, (int)username.length()));
	memcpy(&data[0x20], ip.c_str(), std::min(15, (int)ip.length()));
	SegaCrypto crypto;
	crypto.encrypt(&data[0], 0x30);
	return data;
}
...
std::vector<uint8_t> generateVMI(int fileSize)
{
	...
	unsigned checksum = *(uint32_t *)KEY_FILENAME & 0x41474553; // AND first 4 filename bytes with SEGA
	time_t now;
	time(&now);
	struct tm *tm = localtime(&now);

	ssize_t idx = 0;
	memcpy(&vmiFile[idx], &checksum, 4);
	idx += 4;
	memcpy(&vmiFile[idx], "Daytona Key File", 16);
	idx += 0x20;
	memcpy(&vmiFile[idx], "IONCANNON IONCANNON IONCANNON", 29);
	idx += 0x20;
	tm->tm_year += 1900;
	memcpy(&vmiFile[idx], &tm->tm_year, 2);
	idx += 2;
	vmiFile[idx++] = tm->tm_mon + 1;
	vmiFile[idx++] = tm->tm_mday;
	vmiFile[idx++] = tm->tm_hour;
	vmiFile[idx++] = tm->tm_min;
	vmiFile[idx++] = tm->tm_sec;
	vmiFile[idx++] = tm->tm_wday;
	idx += 2;
	vmiFile[idx] = 1;
	idx = 0x50;
	memcpy(&vmiFile[idx], KEY_FILENAME, strlen(KEY_FILENAME) + 1);
	idx += 8;
	memcpy(&vmiFile[idx], "DAYTONA__KEY", 12);
	idx += 12 + 4;
	memcpy(&vmiFile[idx], &fileSize, 4);
